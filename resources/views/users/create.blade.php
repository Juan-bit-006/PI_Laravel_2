        ...
        <form action="{{ route('users.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="role" class="w-full border rounded-lg px-3 py-2" required>
                    ...
                    <option value="estilista" {{ old('role')=='estilista' ? 'selected' : '' }}>Estilista</option>
            ...
            <div>
                ...
                <select name="estado_login" class="w-full border rounded-lg px-3 py-2" required>
                    <option value="1" {{ old('estado_login', '1') == '1' ? 'selected' : '' }}>Activo</option>
                    <option value="0" {{ old('estado_login') == '0' ? 'selected' : '' }}>Inactivo</option>
                </select>
                @error('estado_login')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
